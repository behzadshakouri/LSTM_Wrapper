...
// Select specific slices (samples)
...
auto KFoldSplit_Cube(const arma::cube& data, const arma::cube& labels, size_t k, size_t fold)
{
    ...
    size_t foldSize = std::max<size_t>(1, n / k);
    size_t start = fold * foldSize;
    size_t end = (fold == k - 1) ? n : std::min(n, start + foldSize);
    ...
    return std::make_pair(
        std::make_pair(SelectSlices(data, trainIdx), SelectSlices(labels, trainIdx)),
        std::make_pair(SelectSlices(data, valIdx), SelectSlices(labels, valIdx)));
}
...
auto KFoldSplit_TimeSeries_Cube(const arma::cube& data, const arma::cube& labels, size_t k, size_t fold)
{
    size_t n = data.n_slices;
    size_t foldSize = std::max<size_t>(1, n / k);
    if (n < 2) throw std::invalid_argument("Not enough samples.");

    size_t trainEnd = std::min((fold + 1) * foldSize, (n > 0 ? n - 1 : 0));
    trainEnd = std::max<size_t>(1, trainEnd);

    size_t valStart = trainEnd;
    size_t valEnd = (fold == k - 1) ? n : std::min(valStart + foldSize, n);

    if (valEnd <= valStart)
        throw std::invalid_argument("Empty validation split.");

    return std::make_pair(
        std::make_pair(data.slices(0, trainEnd - 1), labels.slices(0, trainEnd - 1)),
        std::make_pair(data.slices(valStart, valEnd - 1), labels.slices(valStart, valEnd - 1)));
}
...
bool TrainWithKFold_Cube(
    size_t inputSize, size_t outputSize, size_t rho,
    arma::cube& X, arma::cube& Y,
    double stepSize, size_t batchSize, int epochs, int nFolds, int splitMode,
    const string& outputPath = "./Results/")
{
    const int H1 = 20, H2 = 16, H3 = 14;
    size_t nSamples = X.n_slices;
    if (nSamples < 2) return false;
    if (nSamples < (size_t)nFolds) nFolds = (int)nSamples;

    double trainRatio = 1.0 - 1.0 / double(nFolds);
    cout << "Starting " << nFolds << "-fold cross-validation (mode "
         << splitMode << ", train ratio ≈ " << trainRatio * 100 << "%)...\n";

    vector<double> trainMSEs, valMSEs, trainR2s, valR2s;
    ...
    for (int fold = 0; fold < nFolds; ++fold)
    {
        ...
        size_t maxIter = std::min<size_t>(100, (size_t)epochs * Xtrain.n_slices);
        ens::Adam opt(stepSize, batchSize, 0.9, 0.999, 1e-8, maxIter, 1e-8, true);
        opt.Tolerance() = -1;
        ...
        double mseT = ComputeMSE(predTrain, Ytrain);
        double r2T  = ComputeR2(predTrain, Ytrain);
        double mseV = ComputeMSE(predVal, Yval);
        double r2V  = ComputeR2(predVal, Yval);

        trainMSEs.push_back(mseT); valMSEs.push_back(mseV);
        trainR2s.push_back(r2T);   valR2s.push_back(r2V);

        cout << "  Train MSE: " << mseT << " | R²: " << r2T
             << " | Val MSE: " << mseV << " | R²: " << r2V << endl;
    }

    double avgTrainMSE = arma::mean(arma::vec(trainMSEs));
    double avgValMSE   = arma::mean(arma::vec(valMSEs));
    double avgTrainR2  = arma::mean(arma::vec(trainR2s));
    double avgValR2    = arma::mean(arma::vec(valR2s));

    cout << "\nAverage training   MSE: " << avgTrainMSE
         << " | R²: " << avgTrainR2 << endl;
    cout << "Average validation MSE: " << avgValMSE
         << " | R²: " << avgValR2 << endl;

    ofstream csv(outputPath + "kfold_results.csv");
    ...
    size_t maxIterFinal = std::min<size_t>(100, (size_t)epochs * X.n_slices);
    ens::Adam optF(stepSize, batchSize, 0.9, 0.999, 1e-8, maxIterFinal, 1e-8, true);
    optF.Tolerance() = -1;

    finalModel.Train(X, Y, optF,
                     ens::ProgressBar(),
                     ens::PrintLoss(),
                     ens::EarlyStopAtMinLoss(20));
                 ...
                 int inputsize, int outputsize, bool IO)
{
    ...
    flatData.insert_rows(flatData.n_rows, temp.rows(temp.n_rows - outputsize, temp.n_rows - 1));
    data::Save(filename, flatData);

    cout << "The predicted output (last one) is:\n";
    for (int i = outputsize - 1; i >= 0; --i)
        cout << " (" << flatData(flatData.n_rows - 1 - i, flatData.n_cols - 1) << ")\n";
}

// -------------------------------------------------------------------------
//                    Create Time-Series Cubes
// -------------------------------------------------------------------------
template<typename InputDataType = arma::mat,
         typename DataType = arma::cube,
         typename LabelType = arma::cube>
void CreateTimeSeriesData(InputDataType dataset,
                          DataType& X, LabelType& y,
                          size_t rho, int inputsize, int outputsize, bool IO)
{
    size_t nSamples = dataset.n_cols - rho;
    ...
    for (size_t i = 0; i < nSamples; ++i)
    {
        ...
        if (!IO)
            y.slice(i) = dataset.submat(inputsize, i + 1, inputsize + outputsize - 1, i + rho);
        else
            y.slice(i) = dataset.submat(inputsize - outputsize, i + 1, inputsize - 1, i + rho);
    }
}
...
int main__()
{
    ...
    {
        cerr << "[Error] Final model not found.\n";
        return -1;
    }
    ...
    if (!IO) { testIO.insert_rows(testX.n_rows, testY); trainIO.insert_rows(trainX.n_rows, trainY); }

    cout << "test Data:\n";
    SaveResults(path + "Results/lstm_multi_predictions_test.csv",
                predOutP_Test, scale, testIO, inputSize, outputSize, IO);

    cout << "train Data:\n";
    SaveResults(path + "Results/lstm_multi_predictions_train.csv",
                predOutP_Train, scale, trainIO, inputSize, outputSize, IO);
